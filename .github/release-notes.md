...
- One Discord **Application ID per card** if you want several activities at once: Discord renders a single
  card per application, and a single activity per name, so each extra card needs its own application and
  its own *Shown as* value (see the [wiki: FAQ](https://github.com/quangpao/discord-rp-mac/wiki/FAQ))
